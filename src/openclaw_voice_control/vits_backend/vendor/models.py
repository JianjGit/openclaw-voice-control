...
class StochasticDurationPredictor(nn.Module):
  ...
  def forward(self, x, x_mask, w=None, g=None, reverse=False, noise_scale=1.0):
    ...
    if not reverse:
      ...
      logq = torch.sum(
          -0.5 * (math.log(2 * math.pi) + (e_q ** 2)) * x_mask, [1, 2]) - logdet_tot_q
      logdet_tot = 0
      ...
      nll = torch.sum(
          0.5 * (math.log(2 * math.pi) + (z ** 2)) * x_mask, [1, 2]) - logdet_tot
      return nll + logq
...
class SynthesizerTrn(nn.Module):
  """Classic VITS generator. Training-only discriminators are intentionally omitted."""
  ...
  def infer(self, x, x_lengths, sid=None, noise_scale=1,
